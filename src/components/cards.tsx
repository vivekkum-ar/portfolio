...
const Cards: React.FC<CardsProps> = ({ }) => {
  ...
  const data = [
    {
      title: "Foodies Point - A Restaurant Website",
    ...
    {
      title: "Cherry Berry - A Web Development Service",
    ...
    {
      title: "Recipe Radar - A Recipe App",
    ...
    {
      title: "TheOQ Blog - A Next.js Blog Website",
      description: "A blog website built using MDX and Next.js. This website is a hub of creativity and innovation, dedicated to crafting seamless online experiences with precision and flair. With a focus on cutting-edge technology and meticulous attention to detail, we bring ideas to life with expertise and passion. Our mission is to elevate digital experiences through innovation, delivering solutions that captivate and inspire.",
      content: "https://next-blog-website-phi.vercel.app/",
      footer: "",
...
mm.add("(min-width: 800px)", () => {
  ...
  return (
    <div id='projects' className="container mx-auto px-8 2xl:px-32 lg:mt-32 mt-2 font-lexend">
      {/* <h2 className="text-3xl font-semibold tracking-tight leading-normal">Projects I’m the most proud of</h2> */}
      <h1 className="font-lexend pt-32 mb-16 font-normal text-5xl lg:text-7xl w-full text-center">
          ...
          <Card className='my-12 group shadow-lg flex lg:flex-row flex-col border-2 border-gray-300 dark:border-gray-900'>
            ...
            <CardDescription className='hidden lg:block text-justify'>{item.description}</CardDescription>
              ...
              <CardContent className='flex flex-row justify-between'>
                ...
                <p className={`outline outline-blue-500 rounded-lg px-2 hover:text-blue-500 hidden lg:group-hover:block translate-x-16`}>Hover for preview 👉</p>
              </CardContent>
              <CardFooter>
                <p>{item.footer}</p>
              </CardFooter>
            </div>
            <div className="basis-3/6 px-2.5 flex flex-col relative group" onMouseEnter={() => { setCurrIndex(index); setAnimDirection(true); }} onMouseLeave={() => { setCurrIndex(index); setAnimDirection(false); }}>
              <img src={item.image[0]} alt={item.content} className={`${currIndex == index ? AnimDirection ? "animate-move-and-fade2" : "animate-move-and-fade2-rev" : "animate-move-and-fade2-rev"}  hover:z-40 absolute lazy rounded-2xl h-[170px] lg:h-[90%] border-2 w-full mt-3 image-tilt border-gray-300 dark:border-gray-900`} />
              <img src={item.image[1]} alt={item.content} className={`${currIndex == index ? AnimDirection ? "animate-move-and-fade1" : "animate-move-and-fade1-rev" : "animate-move-and-fade1-rev"} hover:z-40 absolute lazy rounded-2xl h-[170px] lg:h-[90%] border-2 w-full mt-3 image-tilt border-gray-300 dark:border-gray-900`} />
              <img src={item.image[2]} alt={item.content} className=" lazy rounded-2xl h-[170px] lg:h-[90%] border-2 w-full mt-3 image-tilt border-gray-300 dark:border-gray-900" />
            </div>
            <CardDescription className='block lg:hidden text-justify px-4 py-2'>{item.description}</CardDescription>
          </Card>
        )
      })}
